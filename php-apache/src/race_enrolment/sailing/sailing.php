...
if (isset($_POST['submit'])) {
    ...
    while ($insert = mysqli_fetch_assoc($result)) {
        ...
        if ($_POST["$boat_id"] == "DNC" and $_POST["$boat_number"]) {
            array_push($errors, "Cannot select DNC and enter a score for $boat_number");
        }
        if ($_POST["$boat_id"] == "DNF" and $_POST["$boat_number"]) {
            array_push($errors, "Cannot select DNF and enter a score for $boat_number");
        }
    }
    if (count($errors) != 0) {
        ...
        while ($unit_row = mysqli_fetch_assoc($outcome)) {
            ...
            if (mysqli_num_rows($boats) == 0) {
                ...
                while ($boat_row = mysqli_fetch_assoc($boats)) {
                    ...
                    echo "<input type ='radio' name='$boat_id' value='completed'";
                    if ($_POST["$boat_id"] == "completed") {
                        echo " checked";
                    }
                    echo "> completed </input>";
                    echo "<input type ='radio' name='$boat_id' value='DNC'" ;
                    if ($_POST["$boat_id"] == "DNC") {
                        echo " checked";
                    }
                    echo "> DNC </input>";
                    echo "<input type ='radio' name='$boat_id' value='DNF'";
    ...
    while ($completed = mysqli_fetch_assoc($result)) {
      ...
      AND activity_id = '$activity_id' AND race_number = '$race_number' AND unit_id = '$unit_id';";
        ...
        if (mysqli_num_rows($check) == 0) {
            ...
            while ($boat_completed = mysqli_fetch_assoc($boat_result)) {
                ...
                if ($_POST["$unit_id"] == "DNF") {
                    ...
                    $placescore = $base-1;
                    //insert value into table
                    input($conn, $activity_id, $unit_id, 'DNF', $placescore, 'NULL', $event_id, $race_number);
                }
            }
        }
    }
        ...
        if (mysqli_num_rows($check) == 0) {
            ...
            while ($boat_completed = mysqli_fetch_assoc($boat_result)) {
                ...
                if ($_POST["$unit_id"] == "DNC") {
                    ...
                    $placescore = $base-2;
                    //insert value into table
                    input($conn, $activity_id, $unit_id, 'DNC', $placescore, 'NULL', $event_id, $race_number);
                }
            }
        }
    }
    ...
    while ($unit_row = mysqli_fetch_assoc($outcome)) {
        ...
        if (mysqli_num_rows($boats) == 0) {
            ...
            while ($boat_row = mysqli_fetch_assoc($boats)) {
                ...
                echo "<input type ='radio' name='" . $boat_row['boat_id'] . "' value='completed'> completed </input>";
                echo "<input type ='radio' name='" . $boat_row['boat_id'] . "' value='DNC'> DNC </input>";
                echo "<input type ='radio' name='" . $boat_row['boat_id'] . "' value='DNF'> DNF </input>";
                echo "<br>";
            }
        }
    } ?>
